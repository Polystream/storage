...
func (l *LockFile) Touch() error {
	...
	l.stateMutex.Lock()
	if !l.locked || (l.locktype != F_WRLCK) {
...
// if necessary.
func openLock(path string, ro bool) (fd uintptr, err error) {
	flags := O_CLOEXEC | os.O_CREATE
	if ro {
		flags |= os.O_RDONLY
	} else {
		flags |= os.O_RDWR
