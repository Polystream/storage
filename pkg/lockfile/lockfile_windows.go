...
const (
	F_WRLCK   = windows.LOCKFILE_EXCLUSIVE_LOCK
	F_RDLCK   = 0
...
func closeHandle(fd uintptr) {
	windows.Close(windows.Handle(fd))
}

func openHandle(path string, mode int) (uintptr, error) {
	fd, err := windows.Open(path, mode, windows.S_IWRITE)
	return uintptr(fd), err
}

func lockHandle(fd uintptr, lType int16) {
	ol := new(windows.Overlapped)
	for windows.LockFileEx(windows.Handle(fd), uint32(lType), reserved, allBytes, allBytes, ol) != nil {
		time.Sleep(10 * time.Millisecond)
	}
}

func unlockHandle(fd uintptr) {
	ol := new(windows.Overlapped)
